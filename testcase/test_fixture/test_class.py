...
# fixture class 每一个类调用一次
'''
带 autouse=True 的情况
在你的代码中，加上 autouse=True 后，夹具 func 会自动在 TestClassFixTure 
类中的每个测试方法之前运行。你不需要在测试方法中显式地引用 func 夹具。

不带 autouse=True 的情况
如果你不加 autouse=True，则需要在每个测试方法中显式地引用 func 夹具，否则夹具
不会自动运行。
'''
